...
public class SimpleCharacterCreator : MonoBehaviour
{
    ...
    void UpdateCameraPosition()
    {
        ...
        activeCameraPos = Mathf.Clamp01(activeCameraPos+Mouse.current.scroll.ReadValue().x* zoomSpeed * Time.deltaTime);
    ...
    //     {
    //         mouseStartPos = Mouse.current.delta.x.ReadValue()/Screen.width;
    ...
    //     rotationGoal = Mathf.Clamp(mouseRotGoalStart + ((mouseStartPos - (Mouse.current.delta.x.ReadValue() / Screen.width)) * mouseDragSpeedMultiplier), Mathf.Clamp(rotationGoal-4,-180,180), Mathf.Clamp(rotationGoal + 4, -180, 180));
